...
// ---------- 有效角色解析 ----------
// 規則：LINE 群成員（members 有這個 line_user_id）→ 角色跟機器人那邊算法一致：
//       群組沒啟用角色（roles_enabled=false，開放模式）就一律當 owner；
//       啟用後才照 members.role（沒紀錄到的當 caregiver）。
//       不是 LINE 群成員 → 看 access_grants（已兌換 redeemed_at）→ 用 grant 的角色（獸醫等）。
export async function effectiveAccess(groupId, user) {
  ...
  if (user.line_user_id) {
    const { data: m } = await supa
      ...
      .maybeSingle();
    if (m) {
      const { data: g } = await supa.from('groups').select('roles_enabled').eq('id', groupId).maybeSingle();
      const role = g?.roles_enabled ? m.role || 'caregiver' : 'owner';
      return { role, source: 'line', member: m };
    }
  }
...
export async function accessibleGroups(user) {
  ...
  if (user.line_user_id) {
    const { data: ms } = await supa.from('members').select('group_id, role').eq('user_id', user.line_user_id);
    for (const m of ms || []) {
      const { data: g } = await supa.from('groups').select('roles_enabled').eq('id', m.group_id).maybeSingle();
      const role = g?.roles_enabled ? m.role || 'caregiver' : 'owner';
      map.set(m.group_id, { groupId: m.group_id, role, source: 'line' });
    }
  }
