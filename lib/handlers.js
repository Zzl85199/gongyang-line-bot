...
async function handleText(event, t, mention) {
  ...
  if (/^(提醒清單|用藥清單)$/.test(t)) {
    ...
    for (const pet of pets) {
      ...
      else tasks.forEach((tk) => lines.push(`　・${tk.name}（${tk.times.join('、')}）`));
    }
  ...
  if (/^(今天|今日|狀態)$/.test(t)) {
    ...
    return line.reply(event.replyToken, msg.todayStatus(pets, tasksByPet, logs));
  }
  ...
  if ((m = t.match(/^(?:生命之書|回顧)\s*(.*)$/))) {
    const pet = await db.resolvePet(gid, m[1].trim() || null);
  ...
  if (aiText !== null) {
    ...
    return line.reply(event.replyToken, msg.text(out));
  }
...
async function handleImage(event) {
  ...
  await db.addLifebook(pet, { kind: 'memory', photo_path: path, by_name: by });
  return line.reply(event.replyToken, msg.text(`這一刻收進 ${pet.name} 的生命之書了 📖`));
}
