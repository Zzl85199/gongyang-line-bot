...
// 任務卡（小任務 / 提醒卡 / 上傳照片指令卡）送出後呼叫：記住這則訊息的 id，
// 之後使用者「回覆」這則訊息傳照片，才會直接判定成完成任務／確認上傳，不用再問一次。
async function registerPhotoTask(gid, pet, sendResult, taskTitle, kind = 'activity', ttlMinutes = 24 * 60) {
  const msgId = line.lastSentMessageId(sendResult);
  if (!msgId) return null;
  return db.addPendingPhotoTask(gid, pet, msgId, taskTitle, kind, ttlMinutes);
}
...
async function handleText(event, t, mention) {
  ...
  // ---- 指令直接上傳到生命之書：先送出錨點訊息，之後「回覆」這則訊息傳照片就直接收 ----
  if ((m = t.match(/^(?:上傳照片|上傳生命之書|存照片|存生命之書)\s*(.*)$/))) {
    if (!canCheckin()) return line.reply(event.replyToken, msg.noPermCheckin());
    const nm = m[1].trim();
    const pet = nm ? await db.findPetByNameAny(gid, nm) : await db.resolvePet(gid, null);
    if (!pet) return line.reply(event.replyToken, msg.text('先「新增寵物」或「切換 名字」，我才知道要收進誰的生命之書 🐾'));
    const sent = await line.reply(event.replyToken, msg.uploadCommandPrompt(pet));
    await registerPhotoTask(gid, pet, sent, null, 'upload_cmd', 60);
    return sent;
  }
  ...
  if (/^(任務|活動|來個任務|今日任務|建議活動)$/.test(t)) {
    ...
    const sent = await line.reply(event.replyToken, msg.activitySuggestion(pet, act));
    if (act) await registerPhotoTask(gid, pet, sent, act.title);
    return sent;
  }

  // ---- 刪除整個照護圈（不可逆！）先給摘要警告，要求輸入完整確認句才真的刪 ----
  if (/^刪除照護圈$/.test(t)) {
    if (!canManage()) return line.reply(event.replyToken, msg.noPermManage());
    const sum = await db.groupDeletionSummary(gid);
    return line.reply(event.replyToken, msg.deleteGroupWarning(sum));
  }
  if (/^刪除照護圈\s+我確定要刪除$/.test(t)) {
    if (!canManage()) return line.reply(event.replyToken, msg.noPermManage());
    const result = await db.deleteGroupData(gid);
    return line.reply(event.replyToken, msg.deleteGroupDone(result));
  }
  ...
  if (aiText !== null) {
    ...
    const sent = await line.reply(event.replyToken, out);
    if (out.photoTask) {
      const pet = await db.getPet(out.photoTask.petId);
      if (pet) await registerPhotoTask(gid, pet, sent, out.photoTask.title);
    }
    return sent;
  }
...
async function handlePostback(event) {
  ...
  const CHECKIN = new Set(['done', 'undo', 'file', 'morefile', 'unfile', 'walkmood', 'walkdel', 'walkphoto', 'healthpick', 'keep', 'discard', 'delphoto']);
  ...
  // 把剛收到的照片歸到某本圖鑑（快速回覆按下）；e 可以是單一 id，也可以是逗號分隔的多個 id（一次傳很多張時一起歸類）
  if (action === 'file') {
    const idsCsv = data.get('e');
    const key = data.get('c');
    const entries = await db.setLifebookCollectionMany(idsCsv, key);
    if (!entries.length) return line.reply(event.replyToken, msg.text('這些照片找不到了 🐾'));
    const petId = entries[0].pet_id;
    const col = await db.getCollectionMeta(petId, key);
    if (!col) return line.reply(event.replyToken, msg.text('好，先放生命之書就好 🐾'));
    const n = await db.collectionProgress(petId, key);
    ...
    const out = [msg.collectionFiled(idsCsv, col, n, { celebrate, isMilestone: isMs, count: entries.length })];
    // 系列到柔性里程碑 → 自動拼一張回顧卡（安寧/紀念不推）
    if (isMs && celebrate !== 'off') {
      const recapEntries = (await db.lifebookByCollection(petId, key, 9)).filter((e) => e.photo_path);
      if (recapEntries.length) {
        const urls = await Promise.all(recapEntries.map((e) => db.signedPhotoUrl(e.photo_path)));
        out.push(...msg.collectionRecap(pet, col, recapEntries, urls));
      }
    }
    return line.reply(event.replyToken, out);
  }

  // 更多分類 / 改放別本：列出全部圖鑑讓使用者重選（e 同樣可以是多個 id）
  if (action === 'morefile') {
    const idsCsv = data.get('e');
    const entries = await db.getLifebookEntries(idsCsv);
    if (!entries.length) return line.reply(event.replyToken, msg.text('這些照片找不到了 🐾'));
    const cols = await db.listAllCollections(entries[0].pet_id);
    return line.reply(event.replyToken, msg.collectionPicker(idsCsv, cols, { unfile: true, prompt: `要收進哪本圖鑑？（可改放或移出，共 ${entries.length} 張）` }));
  }

  // 移出圖鑑（照片仍留在生命之書）
  if (action === 'unfile') {
    const idsCsv = data.get('e');
    const ids = idsCsv.split(',').map((x) => Number(x)).filter((n) => Number.isFinite(n));
    for (const id of ids) await db.setLifebookCollection(id, null);
    return line.reply(event.replyToken, msg.text('已移出圖鑑，照片還在生命之書 🐾'));
  }

  // 上傳錯了：整張（或整批）從生命之書刪除，連檔案也刪
  if (action === 'delphoto') {
    const idsCsv = data.get('e');
    const deleted = await db.deleteLifebookMany(idsCsv);
    return line.reply(
      event.replyToken,
      msg.text(deleted.length ? `已刪除 ${deleted.length} 張照片，不會留在生命之書了 🗑️` : '這些照片已經不在了。')
    );
  }

  // 沒有回覆任務卡、單純傳照片時的確認：「收進生命之書」
  if (action === 'keep') {
    const idsCsv = data.get('e');
    const entries = await db.getLifebookEntries(idsCsv);
    if (!entries.length) return line.reply(event.replyToken, msg.text('這些照片找不到了 🐾'));
    const pet = await db.getPet(entries[0].pet_id);
    return line.reply(event.replyToken, msg.photoSavedBatch(pet, entries));
  }
  // 「算了，只是聊聊」：整批刪除，不留在生命之書
  if (action === 'discard') {
    const idsCsv = data.get('e');
    await db.deleteLifebookMany(idsCsv);
    return line.reply(event.replyToken, msg.text('好，這幾張就不收進生命之書了，當作聊聊天而已 🐾'));
  }
  ...
  if (action === 'walkphoto') {
    ...
    const pet = await db.getPet(petId);
    if (!pet) return line.reply(event.replyToken, msg.text('找不到這隻毛孩。'));
    await db.setLifebookCollectionMany(idsCsv, 'walk'); // 照片收進「散步紀錄」圖鑑
    const name = await line.getDisplayName(event.source);
  ...
  if (action === 'task') {
    ...
    const pet = petId ? await db.getPet(petId) : await db.resolvePet(gid, null);
    if (!pet) return line.reply(event.replyToken, msg.text('先「新增寵物」或「切換 名字」，再來個小任務吧 🐾'));
    const act = suggestActivity(pet, { lastTitle: last, level: lv });
    const sent = await line.reply(event.replyToken, msg.activitySuggestion(pet, act));
    if (act) await registerPhotoTask(gid, pet, sent, act.title);
    return sent;
  }
...
async function handleImage(event) {
  ...
  // 如果使用者是「回覆」某個小任務卡 / 上傳照片指令卡傳照片，直接視為確認上傳，
  // 用當時任務指定的那隻毛孩，不受期間切換預設對象影響。
  const quotedId = event.message.quotedMessageId || null;
  let pendingTask = quotedId ? await db.findPendingPhotoTask(quotedId) : null;
  if (pendingTask && pendingTask.group_id !== gid) pendingTask = null; // 保險：只認同一個照護圈的任務卡

  // 沒有「回覆」小任務卡 / 上傳照片指令卡的一般照片：不打擾（不存、也不問）。
  // 想收進生命之書，就「回覆」小任務卡傳照片，或先打「上傳照片」再回覆那則訊息傳照片。
  if (!pendingTask) return null;

  const pet = await db.getPet(pendingTask.pet_id);
  ...
  const imageSet = event.message.imageSet || null;
  const entry = await db.addLifebook(pet, {
    kind: pendingTask ? 'task' : 'memory',
    task_title: pendingTask?.task_title || null,
    photo_path: path,
    by_name: by,
  ...
  // 一次選很多張照片傳出時，LINE 會把每一張拆成獨立的 webhook 事件，但都帶同一個 imageSet.id。
  // 只讓「最後一張」（index === total）負責統一回覆，其他張先默默存好、不回應，
  // 這樣同一批照片只會被問一次，不會像現在一樣一張問一輪。
  if (imageSet && imageSet.total > 1) {
    if (imageSet.index !== imageSet.total) return null; // 不是最後一張，交給最後一張統一回覆
    const entries = await waitForImageSet(imageSet.id, imageSet.total);
    return replyPhotoBatch(event.replyToken, pet, entries, pendingTask);
  }

  return replyPhotoBatch(event.replyToken, pet, [entry], pendingTask);
}

// 一次傳很多張時，其他張可能還在別的 webhook 請求裡處理中，稍等一下讓它們存完再一起回覆
// （LINE 各張送達順序不保證，超過等待時間就用當下已存好的張數回覆，不讓使用者等太久）
async function waitForImageSet(imageSetId, total, timeoutMs = 4000, stepMs = 400) {
  const start = Date.now();
  let entries = await db.listLifebookByImageSet(imageSetId);
  while (entries.length < total && Date.now() - start < timeoutMs) {
    await new Promise((r) => setTimeout(r, stepMs));
    entries = await db.listLifebookByImageSet(imageSetId);
  }
  return entries;
}

// 有回覆任務卡 → 意圖明確，直接收，問要不要歸類，並附上完成任務的語氣。
// 沒有回覆任何任務卡的一般照片 → 先問過再收，避免不相關的照片（截圖、菜單…）誤入生命之書；
// 這批照片已經先存好了（含檔案），使用者按「算了」才會整批刪除。
function replyPhotoBatch(replyToken, pet, entries, pendingTask) {
  if (pendingTask) {
    return line.reply(replyToken, msg.photoSavedBatch(pet, entries, { taskTitle: pendingTask.task_title }));
  }
  return line.reply(replyToken, msg.photoConfirmBatch(pet, entries));
}