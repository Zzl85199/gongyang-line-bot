...
async function exec(groupId, toolName, input) {
  ...
  switch (toolName) {
    ...
    case 'add_reminder': {
      ...
      // 去重：同一隻寵物若已有同名提醒，就「更新時間」而不是再插一筆，避免重複任務 → 重複推播
      const existing = await db.findTaskByName(pet.id, input.name);
      if (existing) {
        await db.updateTaskTimes(existing.id, times);
        return r(`已更新 ${pet.name} 的「${existing.name}」時間：${times.join('、')}（原本的同名提醒已覆蓋，不會重複）`);
      }
...
export async function handleAI(groupId, userText) {
  ...
  try {
    const out = prov === 'gemini'
      ? await runGemini(groupId, userText, system, history)
      : await runAnthropic(groupId, userText, system, history);
...
// ---------- Gemini ----------
async function runGemini(groupId, userText, system, history = []) {
...
// ---------- Anthropic / Claude ----------
async function runAnthropic(groupId, userText, system, history = []) {
  const messages = [
