...
async function exec(groupId, toolName, input, perms = { canManage: true, canCheckin: true }) {
  ...
  switch (toolName) {
    ...
    case 'suggest_activity': {
      ...
      const out = r(`${act.title}：${act.prompt}`, [msg.activitySuggestion(pet, act)]);
      out.photoTask = { petId: pet.id, title: act.title };
      return out;
    }
    case 'log_health': {
      const pet = await db.resolvePet(groupId, input.pet_name);
      ...
      const done = [];
      let onlyNote = true;
      if (input.weight != null) { await db.addHealthLog(pet, { kind: 'weight', valueNum: Number(input.weight) }); done.push(`體重 ${input.weight} kg`); onlyNote = false; }
      if (input.appetite) { await db.addHealthLog(pet, { kind: 'appetite', valueText: String(input.appetite) }); done.push(`食慾：${input.appetite}`); onlyNote = false; }
      if (input.symptom) { await db.addHealthLog(pet, { kind: 'symptom', valueText: String(input.symptom) }); done.push(`症狀：${input.symptom}`); onlyNote = false; }
      ...
      // 只有備註（例如洗澡、剪毛這類日常小事）時語氣中性一點，避免聽起來像在講病情；
      // 有實際健康數值（體重/食慾/症狀）才用「健康紀錄」這種比較正式的說法。
      return r(onlyNote ? `已幫 ${pet.name} 記下：${done.join('、')} 📝` : `已記到 ${pet.name} 的健康紀錄：${done.join('、')} 📋`);
    }
    case 'show_health': {
      const pet = await db.resolvePet(groupId, input.pet_name);
      if (!pet) return r('找不到對象寵物');
...
function assemble(outs) {
  ...
  const photoTask = outs.find((o) => o.photoTask)?.photoTask || null;
  if (displays.length) {
    const extra = outs.filter((o) => !o.display && o.result).map((o) => msg.text(o.result));
    const arr = [...extra, ...displays];
    if (photoTask) arr.photoTask = photoTask; // 陣列上掛個屬性，handlers.js 回覆後可以拿來註冊任務照片錨點
    return arr;
  }
