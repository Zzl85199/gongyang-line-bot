...
// 今日狀態與生命之書用 Flex 呈現。
import { fmtTaipeiHHMM, hhmmInTz } from './time.js';
...
// 已經有人打過卡（時間用 DB 的 done_at 依照護圈設定的時區正確轉換 —— 沒傳 tz 時退回台北，
// 但呼叫端應一律傳照護圈的實際時區，才不會跟提醒卡上顯示的時段對不起來）
export function alreadyDone(log, task, slot, tz) {
  const who = log?.done_by_name || '家人';
  const at = tz ? hhmmInTz(new Date(log?.done_at), tz) : fmtTaipeiHHMM(log?.done_at);
...
export function activitySuggestion(pet, act, weekly = false) {
  ...
  const tail =
    act.voice === 'quiet'
      ? '不勉強，有拍到就「回覆」這則訊息傳照片，我會輕輕收進生命之書 📖'
      : '完成後「回覆」這則訊息傳照片，就直接收進生命之書 📖';
  const body = `${head}\n\n${act.title}\n${act.prompt}\n\n${tail}`;
...
// entries：這一批（可能只有一張，也可能是使用者一次選很多張）已存進生命之書的照片。
// taskTitle：有值時代表這批照片是「回覆」某個小任務卡傳的，用慶祝語氣、且不用再問一次「要收進生命之書嗎」。
export function photoSavedBatch(pet, entries, { taskTitle = null } = {}) {
  const ids = entries.map((e) => e.id).join(',');
  const n = entries.length;
  const head = taskTitle
    ? `📸 完成了「${taskTitle}」的小任務！${n > 1 ? `這 ${n} 張` : '這一刻'}收進 ${pet.name} 的生命之書了 📖`
    : n > 1
      ? `這 ${n} 張時刻收進 ${pet.name} 的生命之書了 📖`
      : `這一刻收進 ${pet.name} 的生命之書了 📖`;
  const items = QUICK_PICK.map((key) => {
    const c = getCollection(key);
    return qpItem(`${c.emoji} ${c.title}`, `a=file&e=${ids}&c=${key}`, `收進${c.title}`);
  });
  items.push(qpItem('＋ 更多分類', `a=morefile&e=${ids}`, '更多分類'));
  if (n === 1) items.push(qpItem('🦮 這是散步', `a=walkphoto&e=${ids}&p=${pet.id}`, '這是散步'));
  items.push(qpItem('先不分類', 'a=noop', '先不分類'));
  items.push(qpItem('🗑️ 上傳錯了，刪除', `a=delphoto&e=${ids}`, '刪除這些照片'));
  return {
    type: 'text',
    text: `${head}\n要順手收進哪本圖鑑嗎？`,
    quickReply: { items },
...
// 沒有回覆任何任務卡、單純傳照片進群組時：先問一句再收，避免不相關的照片（截圖、菜單…）誤入生命之書。
// entries 這時已經先存好（含檔案），使用者按「算了」才整批刪除；這樣不用等按鈕就能重新抓一次照片內容。
export function photoConfirmBatch(pet, entries) {
  const ids = entries.map((e) => e.id).join(',');
  const n = entries.length;
  const body = n > 1 ? `收到 ${n} 張照片，要收進 ${pet.name} 的生命之書嗎？📖` : `收到這張照片，要收進 ${pet.name} 的生命之書嗎？📖`;
  return {
    type: 'text',
    text: body,
    quickReply: {
      items: [
        qpItem('🐾 收進生命之書', `a=keep&e=${ids}`, '收進生命之書'),
        qpItem('🗑️ 算了，只是聊聊', `a=discard&e=${ids}`, '算了，只是聊聊'),
      ],
    },
  };
}

// 「上傳照片」指令：送出這則訊息當作任務卡的錨點，之後「回覆」這則訊息傳照片，
// 就會直接收進生命之書（不用再按「要收進嗎」），也不會被其他不相關的照片混進來。
export function uploadCommandPrompt(pet) {
  return {
    type: 'text',
    text: `好，接下來把照片「回覆」這則訊息傳給我，我就直接收進 ${pet.name} 的生命之書 📖\n（1 小時內都算數；沒有回覆這則訊息直接傳的照片，我會先問過你才收）`,
  };
}
...
// entryIds 可以是單一 id，也可以是逗號分隔的多個 id（一次傳很多張照片時，一起改分類）
export function collectionPicker(entryIds, cols, { unfile = false, prompt } = {}) {
  const items = cols.slice(0, 11).map((c) => qpItem(`${c.emoji} ${c.title}`, `a=file&e=${entryIds}&c=${c.key}`, `收進${c.title}`));
  if (unfile) items.push(qpItem('🗂️ 移出圖鑑', `a=unfile&e=${entryIds}`, '移出圖鑑'));
  items.push(qpItem('取消', 'a=noop', '取消'));
  return { type: 'text', text: prompt || '要收進哪本圖鑑呢？', quickReply: { items } };
}

// 歸檔完成。系列：只報張數（無分母）＋柔性里程碑；重要時刻：報第幾個瞬間。
// 一律附「改放別本 / 移出圖鑑 / 整批刪除」讓使用者能修正。慶祝語氣受 celebrate 守門。
// entryIds：單一 id 或逗號分隔多個 id；count：這次一起歸類的張數；n：歸類後的累積張數。
export function collectionFiled(entryIds, col, n, { celebrate = 'on', isMilestone = false, count = 1 } = {}) {
  let body;
  if (col.kind === 'milestone') {
    body = count > 1
      ? `🏅 已記為「重要時刻」的第 ${n - count + 1}～${n} 個瞬間${celebrate === 'off' ? '' : ' ✨'}`
      : `🏅 已記為「重要時刻」的第 ${n} 個瞬間${celebrate === 'off' ? '' : ' ✨'}`;
  } else if (isMilestone && celebrate !== 'off') {
    body = `🎊 ${col.emoji}「${col.title}」${count > 1 ? `又收進 ${count} 張` : '又收進一張'}，越來越完整了！`;
  } else {
    body = count > 1 ? `已收進 ${count} 張到「${col.emoji} ${col.title}」📚` : `已收進「${col.emoji} ${col.title}」📚`;
  }
  return {
    type: 'text',
    text: body,
    quickReply: {
      items: [
        qpItem('放錯了？改放別本', `a=morefile&e=${entryIds}`, '改放別本'),
        qpItem('移出圖鑑', `a=unfile&e=${entryIds}`, '移出圖鑑'),
        qpItem('🗑️ 整批刪除', `a=delphoto&e=${entryIds}`, '整批刪除'),
      ],
    },
  };
}
...
// 刪除整個照護圈：先給摘要警告，這是所有操作裡最重的一個，用「輸入完整確認句」而不是按鈕，
// 避免手滑誤刪（跟按鈕比，打完整句話的門檻高很多，更適合這種不可逆的操作）。
export function deleteGroupWarning(sum) {
  const petLine = sum.petCount ? `　${sum.petNames.join('、')}` : '　（目前沒有毛孩）';
  return text(
    `⚠️ 要刪除這個照護圈嗎？這個動作無法復原，會一次刪掉：\n` +
      `🐾 毛孩 ${sum.petCount} 隻：\n${petLine}\n` +
      `⏰ 提醒 ${sum.taskCount} 個\n` +
      `📖 生命之書照片 ${sum.photoCount} 張（連檔案一起刪）\n` +
      `👥 成員名單 ${sum.memberCount} 人\n` +
      `以及所有健康紀錄、散步日誌、社群貼文…\n\n` +
      `如果確定要刪除，請完整輸入：\n「刪除照護圈 我確定要刪除」`
  );
}

export function deleteGroupDone(result) {
  return text(`已刪除這個照護圈，${result.petCount} 隻毛孩、${result.photoCount} 張照片都清空了。\n如果之後想重新開始，直接「新增寵物 名字」就可以了 🐾`);
}
...
export function help(wakeWord = '小幫手') {
  return text(
    '📖 共養日誌指令\n\n' +
      ...
      '上傳照片 ← 直接把照片收進生命之書（回覆這則訊息傳照片）\n' +
      '安寧 哈吉 / 恢復照護 哈吉 ← 切換安寧期（藥提醒照常、停任務與慶祝）\n' +
      '紀念 旺旺 ← 為離世的毛孩開啟紀念模式\n' +
      '傳照片進群組 → 我會先問要不要收進生命之書，一次傳很多張也只問一次；\n' +
      '回覆「任務」或「上傳照片」那則訊息傳照片，則直接收，不用再問\n\n' +
      '【輪值 / 補提醒】\n' +
      '輪值 爸 媽 我 ← 每天輪一位\n' +
      '過時提醒 30 ← 超過30分沒打卡補提醒（關閉過時提醒）\n\n' +
      '【危險操作】\n' +
      '刪除照護圈 ← 整個照護圈連同所有毛孩、照片、紀錄一起刪除，無法復原\n\n' +
...
export function teaching(wakeWord = '小幫手') {
  return text(
    '🐾 三步驟上手共養日誌\n\n' +
      ...
      '③ 留下回憶\n　傳照片進群組 → 我會問要不要收進生命之書，一次傳很多張只問一次；\n　打「上傳照片」再回覆那則訊息傳照片，就直接收不用再問 📖\n\n' +
      '其他好用的：\n' +
