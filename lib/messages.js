...
// 散步日誌：記一筆後的回覆
export function walkLogged(pet, { place, mood }) {
  const bits = [place || '散步'];
  if (mood) bits.push(mood);
  return text(`🦮 記下了 ${pet.name} 的散步：${bits.join('・')}\n想看紀錄打「散步紀錄 ${pet.name}」，或上網頁編輯。`);
}
// 散步日誌列表
export function walkList(pet, walks) {
  const lines = [`🦮 ${pet.name} 的散步紀錄`];
  if (!walks.length) lines.push('（還沒有紀錄。試試「遛 ' + pet.name + ' 河堤 開心」）');
